...
export default function QurbanConfirmModal({
  ...
  const handleDirectCheckout = () => {
    handleAddToCart();
    toast.success(t('qurbanDetail.confirmModal.toasts.redirectCheckout'));
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                <div className="flex justify-between items-start mb-4">
                  ...
                  >
                    {t('qurbanDetail.confirmModal.title')}
                  </Dialog.Title>
                  <button
                    onClick={onClose}
                ...
                <div className="mt-4">
                  ...
                  <div className="bg-amber-50 rounded-lg p-4 mb-4">
                    <p className="text-sm text-gray-600 mb-1">{t('qurbanDetail.confirmModal.packageLabel')}</p>
                    <p className="font-semibold text-gray-900">{qurbanPackage.name}</p>
                    <div className="mt-2 text-sm text-gray-600 space-y-1">
                      <div className="flex justify-between">
                        <span>{t('qurbanDetail.confirmModal.periodLabel')}</span>
                        <span className="font-medium text-gray-900">{orderData.periodName}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>{t('qurbanDetail.confirmModal.typeLabel')}</span>
                        <span className="font-medium text-gray-900">{animalTypeLabel} - {packageTypeLabel}</span>
                      </div>
                      {qurbanPackage.packageType === 'individual' && (
                        <div className="flex justify-between">
                          <span>{t('qurbanDetail.confirmModal.quantityLabel')}</span>
                          <span className="font-medium text-gray-900">{t('qurbanDetail.confirmModal.quantityValue', { quantity: orderData.quantity })}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-primary-50 rounded-lg p-4 mb-6">
                    <p className="text-sm text-gray-600 mb-2">{t('qurbanDetail.confirmModal.paymentSummary')}</p>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-600">{t('qurbanDetail.confirmModal.summary.subtotal')}</span>
                        <span className="font-medium mono">Rp {formatRupiahFull(subtotal)}</span>
                      </div>
                      <div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">{t('qurbanDetail.confirmModal.summary.adminFee')}</span>
                        ...
                        {qurbanPackage.packageType === 'shared' && qurbanPackage.maxSlots && (
                          <p className="text-xs text-gray-500 mt-1">
                            {t('qurbanDetail.confirmModal.summary.sharedInfo', { maxSlots: qurbanPackage.maxSlots })}
                          </p>
                        )}
                      </div>
                      <div className="border-t border-gray-300 pt-2 mt-2 flex justify-between">
                        <span className="font-semibold text-gray-900">{t('qurbanDetail.confirmModal.summary.total')}</span>
                  ...
                  <p className="text-gray-600 mb-6" style={{ fontSize: '15px' }}>
                    {t('qurbanDetail.confirmModal.chooseAction')}
                  </p>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      {t('qurbanDetail.confirmModal.actions.orderNow')}
                    </Button>
                    ...
                    >
                      ...
                      {t('qurbanDetail.confirmModal.actions.addToCartAndView')}
                    </button>
                    ...
                    >
                      ...
                      {t('qurbanDetail.confirmModal.actions.addToCart')}
                    </button>
                    ...
                    >
                      ...
                      {t('qurbanDetail.confirmModal.actions.browseOther')}
                    </button>
                  </div>
                </div>
              </Dialog.Panel>
