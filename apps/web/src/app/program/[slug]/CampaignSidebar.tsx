...
export default function CampaignSidebar({
  ...
  const isMitraOwner = !!campaign.mitraName && !!campaign.mitraSlug;
  const ownerName = isMitraOwner
    ? campaign.mitraName
    : campaign.organizationName || settings.organization_name;
  const ownerLogoUrl = isMitraOwner
    ? campaign.mitraLogoUrl ||
      settings.organization_institution_logo ||
      settings.organization_logo ||
      '/logo.svg'
    : settings.organization_institution_logo || settings.organization_logo || '/logo.svg';
  const ownerLogoSrc = getImageUrl(ownerLogoUrl, '/logo.svg');
      ...
      <div className="hidden lg:block bg-white rounded-lg shadow-sm p-6 sticky top-24 space-y-6">
        ...
        {ownerName && (
          isMitraOwner && campaign.mitraSlug ? (
            <a
              href={`/mitra/${campaign.mitraSlug}`}
              className="inline-flex flex-col items-start gap-2 hover:bg-gray-50 rounded-lg -mx-2 px-2 py-2 transition-colors"
            >
              <div className="max-w-[30%]">
                <img src={ownerLogoSrc} alt={ownerName} className="w-full h-auto object-contain" />
              </div>
        ...
        <div className="rounded-lg border border-gray-200 p-3 space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium text-gray-900">{formatStartDate(campaign.startDate)}</span>
          </div>
          <div className="flex items-center justify-between text-sm">
        ...
        <div className="space-y-3">
          <div className="flex items-baseline gap-2">
              <span className="text-2xl font-bold text-gray-900">
                Rp{' '}
              {(campaign.collected || 0).toLocaleString(localeTag)}
              </span>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between text-sm text-gray-600">
              <span>
                {t('campaignDetail.sidebar.progress', {
                  percent: progressPercentage.toFixed(1),
                  target: (campaign.goal || 0).toLocaleString(localeTag),
                })}
              </span>
            </div>
          </div>
        ...
        <div className="hidden lg:block">
          ...
          >
            {campaign.pillar?.toLowerCase() === 'fidyah'
              ? t('campaignDetail.sidebar.actions.payFidyahNow')
              : programType === 'wakaf'
                ? t('campaignDetail.sidebar.actions.wakafNow')
                : t('campaignDetail.sidebar.actions.donateNow')}
          </Button>
        </div>
        ...
        <div className="bg-blue-50 rounded-lg p-4">
          <h3 className="text-sm font-semibold text-blue-900 mb-2">
            {t('campaignDetail.sidebar.help.title')}
          </h3>
          <p className="text-sm text-blue-700">
            {t('campaignDetail.sidebar.help.description')}
          </p>
          {settings.organization_whatsapp && (() => {
            ...
            const waMessage = encodeURIComponent(`Saya butuh bantuan/saya mau melaporkan tentang program ${campaign.title}`);
            return (
              <a
              ...
              >
                ...
                {t('campaignDetail.sidebar.help.whatsapp')}
              </a>
            );
          })()}
        </div>
      ...
      {isMounted && createPortal(
        <div className="fixed bottom-0 left-0 right-0 z-[1030] bg-white rounded-t-[20px] shadow-[0_-4px_12px_rgba(0,0,0,0.1)] p-4 lg:hidden">
          <div className="flex justify-between font-bold mb-3">
            <span className="text-gray-900">{t('campaignDetail.sidebar.total')}</span>
          ...
          <div className="flex items-center gap-2">
            <Button onClick={handleDonateClick} size="lg" className="flex-1" disabled={selectedAmount <= 0}>
              {campaign.pillar?.toLowerCase() === 'fidyah'
                ? t('campaignDetail.sidebar.actions.payFidyah')
                : programType === 'wakaf'
                  ? t('campaignDetail.sidebar.actions.wakafNow')
                  : t('campaignDetail.sidebar.actions.donateNow')}
            </Button>
            <button
              ...
              aria-label={t('campaignDetail.sidebar.actions.share')}
            >
            ...
            <button
              ...
              className="p-3 hover:bg-gray-100 rounded-full transition-colors"
              aria-label={t('campaignDetail.sidebar.actions.favorite')}
            >
        ...
        <div className="fixed inset-0 z-[1040] flex items-end justify-center lg:items-center lg:p-4" onClick={() => setIsShareOpen(false)}>
          ...
          <div className="relative bg-white rounded-t-2xl lg:rounded-2xl w-full lg:max-w-md p-6 animate-slide-up" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-gray-900">{t('campaignDetail.sidebar.share.title')}</h3>
            ...
            <div className="grid grid-cols-5 gap-4">
              <button onClick={() => handleShare('whatsapp')} className="flex flex-col items-center gap-2 p-3 hover:bg-gray-50 rounded-lg transition-colors">
                ...
                <span className="text-xs text-gray-600">{t('campaignDetail.sidebar.share.whatsapp')}</span>
              </button>
              <button onClick={() => handleShare('facebook')} className="flex flex-col items-center gap-2 p-3 hover:bg-gray-50 rounded-lg transition-colors">
                ...
                <span className="text-xs text-gray-600">{t('campaignDetail.sidebar.share.facebook')}</span>
              </button>
              <button onClick={() => handleShare('twitter')} className="flex flex-col items-center gap-2 p-3 hover:bg-gray-50 rounded-lg transition-colors">
                ...
                <span className="text-xs text-gray-600">{t('campaignDetail.sidebar.share.twitter')}</span>
              </button>
              <button onClick={() => handleShare('threads')} className="flex flex-col items-center gap-2 p-3 hover:bg-gray-50 rounded-lg transition-colors">
                ...
                <span className="text-xs text-gray-600">{t('campaignDetail.sidebar.share.threads')}</span>
              </button>
              <button onClick={() => handleShare('linkedin')} className="flex flex-col items-center gap-2 p-3 hover:bg-gray-50 rounded-lg transition-colors">
                ...
                <span className="text-xs text-gray-600">{t('campaignDetail.sidebar.share.linkedin')}</span>
              </button>
            </div>
          </div>
        </div>,
