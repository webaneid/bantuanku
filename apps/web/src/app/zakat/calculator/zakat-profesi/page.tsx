...
export default function ZakatProfesiPage() {
  ...
  const loadConfig = async () => {
    try {
      ...
      const profesiType = zakatTypesData.find((t: ZakatType) =>
        t.slug === 'profesi' ||
        t.slug === 'zakat-profesi' ||
        t.name?.toLowerCase().includes('profesi')
      );
      setZakatType(profesiType || null);
  ...
  const displayName = displayMeta?.name || zakatType?.name || t('zakatCalculator.profesi.defaultName');
      ...
      <main className="min-h-screen bg-gray-50 py-8 md:py-12">
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="mb-6">
              <h2 className="text-xl font-semibold text-gray-900">{t('zakatCalculator.profesi.calculatorTitle')}</h2>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-emerald-50 border-l-4 border-emerald-500 p-4 rounded">
                <p className="text-sm font-semibold text-emerald-900 mb-2">
                  {t('zakatCalculator.profesi.nishabMonthly', { amount: formatCurrency(nishabProfesi) })}
                </p>
                <p className="text-xs text-emerald-700">
                  {t('zakatCalculator.profesi.nishabHelper', { grams: nishabGoldGrams })}
                </p>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {t('zakatCalculator.profesi.fields.monthlyIncome')}
                </label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {t('zakatCalculator.profesi.fields.bonus')}
                </label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">Rp</span>
                  <input
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {t('zakatCalculator.profesi.fields.otherIncome')}
                </label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">Rp</span>
                  <input
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <p className="text-sm font-medium text-gray-700 mb-1">{t('zakatCalculator.profesi.totalIncomeLabel')}</p>
                <p className="text-xl font-bold text-gray-900">{formatCurrency(totalPenghasilan)}</p>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {t('zakatCalculator.profesi.fields.expenses')}
                </label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">Rp</span>
                  <input
                ...
                <p className="mt-1 text-sm text-gray-500">
                  {t('zakatCalculator.profesi.expensesHelper')}
                </p>
              </div>

              {/* Penghasilan Bersih */}
              <div className="bg-primary-50 border border-primary-200 rounded-lg p-4">
                <p className="text-sm font-medium text-primary-900 mb-1">{t('zakatCalculator.profesi.netIncomeLabel')}</p>
                <p className="text-xl font-bold text-primary-700">{formatCurrency(penghasilanBersih)}</p>
                {penghasilanBersih < nishabProfesi && (
                  <p className="text-xs text-orange-600 mt-2">
                    {t('zakatCalculator.common.belowNisab', { amount: formatCurrency(nishabProfesi) })}
                  </p>
                )}
              </div>
                ...
                <>
                  <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 border border-emerald-200 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-sm font-bold text-emerald-900">{t('zakatCalculator.profesi.zakatAmountLabel')}</p>
                      <button
                        type="button"
                        onClick={handleCopy}
