...
export default function ZakatPaymentDetailPage() {
  const router = useRouter();
  const params = useParams();
  const { user, isHydrated } = useAuth();
  const [selectedMethod, setSelectedMethod] = useState<PaymentMethod | null>(null);
  const [availableMethods, setAvailableMethods] = useState<PaymentMethod[]>([]);
  const [donations, setDonations] = useState<ZakatDonation[]>([]);
    ...
    try {
      ...
      await Promise.all(
        donations.map((donation) =>
          api.post(`/zakat/donations/${donation.id}/confirm-payment`, {
      ...
      const uploadPromises = donations.map(async (donation) => {
        ...
        const response = await fetch(`${API_URL}/zakat/donations/${donation.id}/upload-proof`, {
          method: 'POST',
          body: formData,
        });
    ...
    <div className="space-y-6">
      ...
      {availableMethods.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-8 text-center">
          ...
          <Link href={`/account/zakat/${params.id}/payment-method`} className="mt-4 inline-block">
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-3">
            <Link href={`/account/zakat/${params.id}/payment-method`} className="flex-1">
