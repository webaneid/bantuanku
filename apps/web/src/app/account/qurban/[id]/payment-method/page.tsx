...
export default function QurbanPaymentMethodPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (methodsData.success) {
        ...
        // Filter methods for qurban program
  ...
  const handleSelectMethod = (methodType: string) => {
    if (methodType === 'bank_transfer' || methodType === 'qris') {
      ...
      router.push(`/account/qurban/${params.id}/payment-detail`);
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Back Button */}
      <div>
        <Link href={`/account/qurban/${params.id}`}>
          <Button variant="outline" size="lg" className="w-full">
            Kembali ke Detail Qurban
          </Button>
        </Link>
      </div>
    </div>
