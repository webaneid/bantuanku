...
export default function QurbanPaymentDetailPage() {
  const router = useRouter();
  const params = useParams();
  const { user, isHydrated } = useAuth();
  const [selectedMethod, setSelectedMethod] = useState<PaymentMethod | null>(null);
  const [availableMethods, setAvailableMethods] = useState<PaymentMethod[]>([]);
  const [orders, setOrders] = useState<QurbanOrder[]>([]);
  ...
  useEffect(() => {
    ...
    if (!methodType || !pendingDonationsStr) {
      ...
      router.push(`/account/qurban/${params.id}`);
      return;
    }
  ...
  const loadPaymentData = async (methodType: string, donationData: Array<{id: string, program: string}>) => {
    try {
      // Fetch qurban orders
      const orderPromises = donationData.map(async (d) => {
        const response = await api.get(`/account/donations/${d.id}`);
      ...
      if (data.success) {
        ...
        // Filter by program (qurban)
        const filteredMethods = filterMethodsByPrograms(filteredByType);
  ...
  const filterMethodsByPrograms = (methods: PaymentMethod[]) => {
    const program = 'qurban';

    // Step 1: Find methods that specifically match qurban program (NOT general)
    const specificMatches = methods.filter((method) => {
      ...
      // Check if method's programs include qurban
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      // Step 1: Confirm payment method for all orders
      await Promise.all(
        orders.map((order) =>
          api.post(`/qurban/orders/${order.id}/confirm-payment`, {
            paymentMethodId: selectedMethod.code,
          })
        )
      );

      // Step 2: Upload payment proof for all orders
      const uploadPromises = orders.map(async (order) => {
        const formData = new FormData();
        formData.append('file', paymentProof);

        const response = await fetch(`${API_URL}/qurban/orders/${order.id}/upload-proof`, {
          method: 'POST',
          body: formData,
        });

        if (!response.ok) {
          throw new Error(`Failed to upload proof for order ${order.id}`);
        }
      ...
      // Redirect back to qurban detail
      router.push(`/account/qurban/${params.id}`);
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      {availableMethods.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-8 text-center">
          ...
          <Link href={`/account/qurban/${params.id}/payment-method`} className="mt-4 inline-block">
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-3">
            <Link href={`/account/qurban/${params.id}/payment-method`} className="flex-1">
