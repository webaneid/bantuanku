...
export default function ActivityReportsPage() {
  ...
  const { data: reports, isLoading } = useQuery({
    queryKey: ["activity-reports", statusFilter],
    queryFn: async () => {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(error.response?.data?.message || "Gagal menghapus laporan kegiatan");
    },
  });
    ...
    <div className="dashboard-container">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Kelola laporan kegiatan campaign</p>
        </div>
        <button
          onClick={() => router.push("/dashboard/campaigns/activity-reports/create")}
              ...
              {paginatedReports && paginatedReports.length > 0 ? (
                paginatedReports.map((report: any) => (
                  <tr key={report.id}>
                    ...
                    <td>
                      <div className="text-sm text-gray-600">{report.campaign?.title || "-"}</div>
                    </td>
                    <td>
                    ...
                    <td>
                      <div className="table-actions">
                        <button
                          onClick={() => router.push(`/dashboard/campaigns/activity-reports/${report.id}`)}
                        ...
                        <button
                          onClick={() => router.push(`/dashboard/campaigns/activity-reports/${report.id}/edit`)}
              ...
              <div key={report.id} className="table-card">
                <div className="table-card-header">
                  <div className="table-card-header-left">
                    ...
                    <div className="table-card-header-subtitle">{report.campaign?.title || "-"}</div>
                  </div>
