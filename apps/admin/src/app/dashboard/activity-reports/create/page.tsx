...
export default function CreateActivityReportPage() {
  ...
  // Fetch qurban period detail (for auto-filling animal counts)
  const { data: qurbanPeriodDetail } = useQuery({
    queryKey: ["qurban-period-detail", referenceId],
    queryFn: async () => {
      const response = await api.get(`/admin/qurban/periods/${referenceId}/detail`);
      return response.data;
    },
    enabled: referenceType === "qurban_period" && !!referenceId,
  });
  ...
  const handleReferenceChange = (value: string) => {
    ...
    // Reset typeSpecificData so useEffect can re-fill from API
    if (referenceType === "qurban_period") {
      setTypeSpecificData({});
    }
  };

  // Auto-fill qurban period stats when detail is loaded
  useEffect(() => {
    if (qurbanPeriodDetail?.stats && referenceType === "qurban_period" && Object.keys(typeSpecificData).length === 0) {
      const stats = qurbanPeriodDetail.stats;
      setTypeSpecificData({
        animals_by_type: {
          kambing: stats.totalGoats || 0,
          sapi: stats.totalCows || 0,
          domba: 0,
        },
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      setFeedback({
        ...
        type: "error",
        title: "Gagal",
        message: error.response?.data?.message || "Gagal membuat laporan kegiatan",
      });
    },
  });
      ...
      <div className="form-page-header">
        ...
        <div className="form-page-header-content">
          ...
          <p className="form-page-subtitle">
            Buat laporan kegiatan untuk Campaign, Zakat, atau Qurban
          </p>
        </div>
      </div>
