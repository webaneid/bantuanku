...
export default function QurbanOrdersPage() {
  ...
  const { data: packagesData } = useQuery({
    queryKey: ["admin-qurban-package-options"],
    queryFn: async () => {
  ...
  const packageOptions = useMemo(() => {
    const base = [{ value: "", label: "Semua Paket" }];
    if (!packagesData) {
      return base;
    }
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (packageFilter) params.product_id = packageFilter;
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Orders Qurban</h1>
          <p className="text-gray-600 mt-1">Semua transaksi order qurban</p>
        </div>
        <button
          type="button"
          className="btn btn-primary btn-md"
          onClick={() => router.push("/dashboard/transactions/create?product_type=qurban")}
        >
      ...
      <div className="filter-container">
        <div className="filter-group">
          <Autocomplete
            ...
            value={packageFilter}
            onChange={handleFilterChange(setPackageFilter)}
          ...
          transactions.map((transaction: any) => (
            <div key={transaction.id} className="table-card">
              ...
              <div className="table-card-row">
                <span className="table-card-row-label">Paket Qurban</span>
