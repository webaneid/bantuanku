...
export default function ImportMustahiqModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const handleFileChange = (selectedFile: File | null) => {
    ...
    if (!ext || !["xlsx", "csv"].includes(ext)) {
      setErrorMsg("Format file harus .xlsx atau .csv");
      return;
    }
    setErrorMsg(null);
  ...
  const downloadTemplate = async () => {
    const res = await api.get("/admin/mustahiqs/import/template", { responseType: "blob" });
    const url = URL.createObjectURL(new Blob([res.data]));
    const a = document.createElement("a");
    a.href = url;
    a.download = "template-import-mustahiq.xlsx";
    a.click();
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await api.post("/admin/mustahiqs/import/preview", fd, {
  ...
  const handleCommit = async () => {
    ...
    try {
      const fd = new FormData();
      fd.append("file", file);
      const res = await api.post(`/admin/mustahiqs/import/commit?mode=${mode}`, fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });
            ...
            <div className="space-y-5">
              <div className="flex items-start justify-between">
                <p className="text-sm text-gray-600">
                  Upload file Excel (.xlsx) atau CSV. Wajib: Nama dan Kategori Asnaf.
                  Deduplication berdasarkan No. Identitas (jika diisi).
                </p>
                <button
                  type="button"
                ...
                <div className="flex gap-3 mt-1">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-sm">
                      <strong>Skip</strong> — lewati mustahiq yang sudah ada
                    </span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                    ...
                    />
                    <span className="text-sm">
                      <strong>Update</strong> — perbarui data yang sudah ada
                    </span>
                  </label>
                </div>
              </div>
              ...
              >
                ...
                />
                <ArrowUpTrayIcon
                  className={`w-10 h-10 mx-auto mb-3 ${file ? "text-green-500" : "text-gray-400"}`}
                />
                {file ? (
                  <div>
              ...
              {errorMsg && (
                <p className="text-sm text-red-600 flex items-center gap-1">
                  <XCircleIcon className="w-4 h-4" /> {errorMsg}
                </p>
              )}
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              {previewData.duplicateRows > 0 && (
                <div className="text-sm p-3 rounded-lg bg-yellow-50 border border-yellow-200 text-yellow-800">
                  {previewData.duplicateRows} mustahiq duplikat (berdasarkan No. Identitas) akan{" "}
                  <strong>{mode === "skip" ? "dilewati" : "diperbarui"}</strong>.
                </div>
              )}
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Asnaf</th>
                    ...
                    {filteredRows?.map((row) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-gray-600">
                          {row.data.asnafCategory ? ASNAF_LABELS[row.data.asnafCategory] ?? row.data.asnafCategory : "-"}
                        </td>
          ...
          {step === "preview" && (
            <>
              ...
              <button
                ...
                disabled={
                  isLoading ||
                  (previewData?.validRows === 0 && previewData?.duplicateRows === 0)
                }
              >
                {isLoading
                  ? "Mengimport..."
